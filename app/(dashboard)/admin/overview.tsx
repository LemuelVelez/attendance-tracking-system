...
import { getAllData, type User, type Event, type Attendance, type FineDocument } from "@/lib/overview/overview"
import { Loader2, Users, BarChartIcon, Calendar, DollarSign, TrendingUp, List } from "lucide-react"
...
export default function Overview() {
  ...
  const averageAttendance = totalEvents > 0 ? ((totalAttendance / (totalEvents * totalStudents)) * 100).toFixed(2) : 0
      ...
      (acc, curr) => {
        if (!acc.some((event) => event.eventName === curr.eventName)) {
          acc.push({
            ...
            attendance: data.attendance.filter((a) => a.eventName === curr.eventName).length,
            total: totalStudents,
          })
        }
        return acc
  ...
  const paginatedEvents = recentEvents.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage)
  ...
  return (
    <div className="p-4 sm:p-8 space-y-4 sm:space-y-8 min-h-screen bg-gradient-to-br from-background to-secondary/20 background-with-overlay">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-primary">Overview</h1>
        ...
        <div className="grid gap-4 sm:gap-6 md:grid-cols-2 mt-8 sm:mt-12">
          ...
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-lg sm:text-xl font-semibold text-primary">Top Events by Attendance</CardTitle>
          ...
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-lg sm:text-xl font-semibold text-primary">Attendance Trend</CardTitle>
        ...
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-lg sm:text-xl font-semibold text-primary">Recent Events</CardTitle>
            <List className="h-5 w-5 text-primary" />
          </CardHeader>
          <CardContent>
            <ScrollArea className="h-[400px] w-full overflow-auto touch-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[120px] sm:w-[250px]">Event</TableHead>
                ...
                <TableBody>
                  {paginatedEvents.map((event) => (
                    <TableRow key={event.eventName} className="hover:bg-muted/50 transition-colors">
